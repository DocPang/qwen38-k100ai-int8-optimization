"""TP1 DFlash2 Final long-KV BM128/w8 attention v2 + exact 257900 qtail.

The validated TP1 early-Triton/q8split stack is installed first.  This overlay intercepts
only SGLang's pack-paged-kv-to-varlen fast path for batch1 q=8192 prefill when
32K <= KV <= 248K on the scheduler 8K grid on TP1 QH24/KVH4/D256 BF16/page64 geometry.
...
_PARENT = f"{_ROOT}/runtime_patch_dflash_early_triton_v1/sitecustomize.py"
_SRC = f"{_ROOT}/external_images/sglang0.5.12-20260620-src/flash_attn/flash_attn_triton_mqa_gqa.py"

runpy.run_path(_PARENT, run_name="__q38_tp1_dflash2_longkv_v2_parent__")
...
if raw.count(old_start) != 1 or raw.count(old_guard) != 1:
    raise RuntimeError("TP1 longkv v2 vendor Triton causal source changed; fail closed")
raw = raw.replace(old_start, new_start).replace(old_guard, new_guard)
tmp_src = f"/tmp/q38_tp1_dflash2_longkv_v2_{os.getpid()}.py"
Path(tmp_src).write_text(raw)
spec = importlib.util.spec_from_file_location(f"q38_tp1_dflash2_longkv_v2_{os.getpid()}", tmp_src)
...
def _eligible(*, q, forward_batch, metadata, layer, window_size, sinks, key_cache, value_cache, page_size):
    ...
    if bool(getattr(forward_batch, "mha_return_lse", False)):
        return False, None
    if not q.is_contiguous() or q.numel() != 8192 * 24 * 256:
        return False, None
...
def _try_qtail(*, q, forward_batch, metadata, layer, window_size, sinks, key_cache, value_cache, page_size):
    ...
    _qtail_probes += 1
    if _qtail_probes <= 4:
        print(f"[K100 DFlash2 TP1 qtail probe] q={qn} kv={kv} maxq={int(getattr(metadata,'max_seq_len_q',-1))} maxk={int(getattr(metadata,'max_seq_len_k',-1))}", flush=True)
    exact = (
        ...
        and q.is_contiguous() and q.numel() == 3948 * 24 * 256
    ...
    if int(pk.shape[0]) != 257900 or int(pv.shape[0]) != 257900:
        raise RuntimeError(f"TP1 qtail v2 gather mismatch K={tuple(pk.shape)} V={tuple(pv.shape)}")
    _qtail_hits += 1
    if _qtail_hits <= 4 or _qtail_hits % 16 == 0:
        print(f"[K100 DFlash2 TP1 qtail BM128 v2] ACTIVE hit={_qtail_hits} layer={int(getattr(layer,'layer_id',-1))} q=3948 kv=257900 ceil-causal", flush=True)
    out = _tri_fn(
...
def _longkv_try(*, q, forward_batch, metadata, layer, window_size, sinks,
    ...
    if int(pk.shape[0]) != kv or int(pv.shape[0]) != kv:
        raise RuntimeError(f"TP1 longkv v2 gather mismatch kv={kv} K={tuple(pk.shape)} V={tuple(pv.shape)}")
    if tuple(pk.shape[1:]) != (4, 256) or tuple(pv.shape[1:]) != (4, 256):
        raise RuntimeError(f"TP1 longkv v2 packed geometry drift K={tuple(pk.shape)} V={tuple(pv.shape)}")

    _hits += 1
    if _hits <= 8 or _hits % 16 == 0:
        print(
            "[K100 DFlash2 TP1 longkv BM128 v2] ACTIVE "
            f"hit={_hits} layer={int(getattr(layer, 'layer_id', -1))} q=8192 kv={kv} "
            "qh=24 kvh=4 d=256 BM128/BN64/w8",
            flush=True,
        )
...
print(
    "[K100 DFlash2 TP1 longkv BM128 v2] installed after TP1 early-Triton/q8split parent; "
    "direct pack hook q8K 32K<=KV<=248K plus exact q3948/KV257900 ceil-causal tail QH24/KVH4/D256 BF16; "
