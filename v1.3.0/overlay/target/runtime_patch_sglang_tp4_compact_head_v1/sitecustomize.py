"""TP4 compact-head candidate on top of TP4-V1 rank-local W8A8 stack.

For M=1 decode only, each TP4 rank replaces its local BF16 lm_head GEMM
[1,5120] x [62080,5120] with:
...
those four local dense shards into the normal full-vocab logits tensor.
...
_BASE=("/data/qwen38-27b-k100ai-int8-opt/"
      "runtime_patch_sglang_tp4_k5120_ldsx_v1/sitecustomize.py")
runpy.run_path(_BASE,run_name="__q38_sglang_tp4_ranklocal_parent__")

if os.getenv("SGLANG_Q38_TP4_COMPACT_HEAD_M1","0")=="1":
    ...
    _FAST_CTX=contextvars.ContextVar("q38_sglang_tp4_compact_head_fast",default=False)
    _seen_fast=False
    ...
    def _frozen_indices(device=None):
        ...
        if int(idx.numel())!=K_SELECTOR: raise RuntimeError(f"TP4 compact head index cardinality drift {idx.numel()}")
        digest=hashlib.sha256(idx.numpy().tobytes()).hexdigest()
        if digest!=FROZEN_INDEX_SHA: raise RuntimeError(f"TP4 compact head index SHA drift {digest}")
        return idx.to(device).contiguous() if device is not None else idx.contiguous()

    def _build_selector(head):
        if hasattr(head,"k100_q38_tp4_selector_weight"): return
        ...
        if not isinstance(w,torch.Tensor) or w.dtype is not torch.bfloat16 or tuple(w.shape)!=(VOCAB_LOCAL,K_FULL):
            raise RuntimeError(f"TP4 compact head unexpected lm_head dtype={getattr(w,'dtype',None)} shape={getattr(w,'shape',None)}")
        ...
        head.register_buffer("k100_q38_tp4_selector_idx",idx,persistent=False)
        head.register_buffer("k100_q38_tp4_selector_weight",q,persistent=False)
        head.register_buffer("k100_q38_tp4_selector_scale",s,persistent=False)
        torch.cuda.synchronize()
        print(f"[K100 SGLang TP4 compact head] selector ready local_shape={tuple(q.shape)} topk={TOPK} bytes={q.numel()+s.numel()*4} build_s={time.perf_counter()-t0:.3f}",flush=True)
    ...
    def _compact_local(hidden_states,head):
        ...
        if tuple(x.shape)!=(1,K_FULL): raise RuntimeError(f"TP4 compact head expected [1,{K_FULL}], got {tuple(x.shape)}")
        xq,xs=per_token_quant_int8(x)
        xsub=torch.index_select(xq,1,head.k100_q38_tp4_selector_idx).contiguous()
        approx=matmul_int8(xsub,xs,head.k100_q38_tp4_selector_weight.t(),head.k100_q38_tp4_selector_scale,torch.bfloat16,SELECTOR_CONFIG)
    ...
    def _metadata_safe(self,hidden_states,lm_head,logits_metadata,embedding_bias):
        try:
            ...
            if not hasattr(lm_head,"k100_q38_tp4_selector_weight"): return False
    ...
    def _compute_lm_head_wrapped(self,hidden_states,lm_head,embedding_bias=None):
        ...
        if _FAST_CTX.get(False):
            if not _seen_fast:
                _seen_fast=True; print("[K100 SGLang TP4 compact head] ACTIVE local Top512 + native TP all-gather",flush=True)
    ...
    print("[K100 SGLang TP4 compact head] installed; shortlist candidate, native TP gather preserved",flush=True)
