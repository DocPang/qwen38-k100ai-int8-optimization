...
from sglang.srt.utils.common import ceil_align, empty_device_cache, raise_error_or_warn
...
class SchedulerRuntimeCheckerMixin:
    ...
    def _check_full_pool(
        ...
        if leak and self.page_size > 1:
            # Failure-only physical-page diagnosis.  The allocator free lists are
            # page ids, while radix values are token-slot ids; normalize both to
            # page ids before comparing ownership.  Do not use the cached
            # evictable counter as authority here -- that is exactly what this
            # probe is meant to validate.
            allocator = self.token_to_kv_pool_allocator
            free_pages = set(allocator.free_pages.tolist())
            free_pages.update(allocator.release_pages.tolist())
            tree_values = self.tree_cache.all_values_flatten()
            tree_pages = set((tree_values // self.page_size).tolist())
            tree_pages.discard(0)
            expected_pages = set(range(1, allocator.num_pages + 1))
            free_tree_overlap = free_pages & tree_pages
            missing_physical_pages = expected_pages - free_pages - tree_pages
            extra_physical_pages = (free_pages | tree_pages) - expected_pages
            actual_tree_tokens = len(tree_pages) * self.page_size
            counter_delta_tokens = ps.full_evictable_size - actual_tree_tokens
            msg += (
                f", page_diag_free_pages={len(free_pages)}"
                f", page_diag_actual_tree_pages={len(tree_pages)}"
                f", page_diag_actual_tree_tokens={actual_tree_tokens}"
                f", page_diag_evictable_counter_delta_tokens={counter_delta_tokens}"
                f", page_diag_free_tree_overlap={sorted(free_tree_overlap) or None}"
                f", page_diag_missing_physical_pages={sorted(missing_physical_pages) or None}"
                f", page_diag_extra_physical_pages={sorted(extra_physical_pages) or None}"
            )
        return leak, msg
    ...
    def on_idle(self: Scheduler):
        ...
        # Trim allocator high-water cache on every scheduler rank.  This is
        # intentionally separate from IdleSleeper: only the ingress rank owns
        # the ZMQ poller, but every TP/PP rank owns an independent GPU allocator
        # that can retain transient FA/Triton/speculative workspaces after a
        # request.  Logical KV/Radix/Mamba pools stay intact.
        empty_cache_interval = envs.SGLANG_EMPTY_CACHE_INTERVAL.get()
        if empty_cache_interval > 0:
            now = time.monotonic()
            last = getattr(self, "_all_rank_empty_cache_last_time", None)
            if last is None:
                self._all_rank_empty_cache_last_time = now
            elif now - last >= empty_cache_interval:
                self._all_rank_empty_cache_last_time = now
                empty_device_cache(self.device_module)
                logger.info(
                    "Idle allocator cache trimmed on rank=%s gpu_id=%s interval=%.3fs",
                    getattr(self, "tp_rank", None),
                    getattr(self, "gpu_id", None),
                    empty_cache_interval,
                )

        # sleep until next event (ingress rank only).  GPU allocator trimming is
        # all-rank and intentionally does not depend on --sleep-on-idle.
