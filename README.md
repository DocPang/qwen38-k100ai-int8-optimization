...
> **算力服务器不能联网？** 请直接看：[离线算力服务器部署教程（Docker save/load，约 5 分钟读完）](OFFLINE_DEPLOY.md)
...
4. 把 4 个预编译 `.so` 放入固定 runtime 路径并逐个检查；
5. 保存 U036 所需的 SourceFind Triton GQA 原始实现并设置运行时入口。
